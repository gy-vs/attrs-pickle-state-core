...
def define(
    ...
    *,
    ...
    Please note that *getstate_setstate* defaults to `None` like in
    `attr.s`; because *slots* defaults to `True` here, ``__getstate__`` and
    ``__setstate__`` are therefore generated by default.
...
mutable = define
frozen = partial(define, frozen=True)
